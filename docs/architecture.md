...
`XRInteractionCoordinator` owns the queued input boundary and per-controller interaction state. Explicit modes include `IDLE`, `MENU_OPEN`, `SPAWN_PREVIEW`, `RAY_INTERACTING`, `GRIP_TRANSFORMING`, and `STICK_EQUIPPED`. Radial-menu state explicitly separates parent and child selected indices, roll quaternions/baselines, dial presentation, the latched parent, opening world transform, signed pull axis, and projected pull distance. It routes intent to injected handlers; it does not know synth, morph, or timeline internals.
...
Manual Honk squeeze acquisition requires the registered `honk.squeeze` / `HIT_horn` target even for locked or gripped Honks. A validated capture survives off-target roll until release or target invalidation. Ray queries batch authoritative pose work per owner and refresh transforms on every query; only adjacent input routing shares a hit. Manual voice starts use membership transitions plus the audio service's active/pending status. See the [manual response repair and browser benchmark](manual-honk-response-repair.md).

The spawn hierarchy is data-driven. `config/spawning.js` defines ordered category records containing leaf IDs; `SpawnCatalog` validates that every leaf exists, is visible, and occurs only once, then resolves each ID to the canonical frozen catalog object. Categories are not catalog commands. `SpawnMenuController` owns parent/child transitions and leaf-only confirmation, while `RadialSpawnMenu` builds and caches one parent ring plus one child ring per category and owns roll/pull measurement and presentation. Formation leaves—including the temporary two-octave preset—continue into `FormationSpawner`, `SpawnPreview`, and `SpawnPlacementController` without a second placement or persistence model.

On open, the radial view transforms local menu normal `(0, 0, 1)` by the controller's opening world quaternion. It compares that axis with `viewerWorldPosition - openingControllerWorldPosition` and negates the axis when their dot product is negative. Every frame, it removes headset translation from controller translation before projection: `controllerOnlyDelta = (currentControllerWorldPosition - openingControllerWorldPosition) - (currentViewerWorldPosition - openingViewerWorldPosition)`, then `pullDistance = dot(controllerOnlyDelta, signedOpeningNormal)`. Shared head/body-and-controller movement therefore does not advance the menu, and room/global Z never participates. The menu root separately follows the controller parallel to the opening radial plane while its displacement along that opening normal is projected out, keeping menu depth fixed during the pull. The configured entry/exit thresholds are 0.05 m and 0.035 m; the child layer is offset 0.055 m along the corresponding signed local menu Z.
