...
function add(id, type, title, instruction, options = {}) {
  steps.push(Object.freeze({ id, type, title, instruction, prerequisites: steps.length ? [steps.at(-1).id] : [],
    entry: 'Demonstrate or Practice this step; Next prepares prerequisites. Only fresh learner evidence earns practice credit.',
...
for (const phrase of Object.keys(C.phrases)) add(`phrase-${phrase}`,'phrase',`Practice phrase ${phrase}`,
  `Follow the next note on the panel and the rings. Practice counts in at a backing boundary. ${phrase==='D'?'Let the final C linger.':'Keep each descending Eb → C glide in one voice.'}`,
  {phrase,timed:true,beats:16});
add('performance','performance','Perform the whole study','A, B, A, C, B, D · six connected phrases. Practice counts you in with both backing parts. Leave the last beat silent.',{timed:true,beats:96});
