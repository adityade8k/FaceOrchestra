...
export class TutorialLessonFlow {
  constructor(t){this.t=t;this.a=t.adapter;this.generation=0;this.practiceCapture=null;this.ownedPlayback=new Set();}
  snapshots(){
  ...
  takeStamp(h){return `${h?.id}:${h?.looperData.takeRevision}:${h?.looperData.activityRevision}`;}
  demoOwnsTake(demo,role){return demo.expectedTakes[role]===this.takeStamp(this.a.get(role));}
  demoOwnsTakes(demo){return Object.keys(demo.savedTakes).every(role=>this.demoOwnsTake(demo,role));}
  ...
  finalizeCapture(session,now=performance.now()){
    ...
    // Never send idle Stop: ordinary idle Stop deliberately clears a take.
    if(h.transport.recording||h.transport.recordArmed)this.a.command(`stop-record-${session.step.looperRole}`,now,session.origin);
    this.a.snapshotAt=-Infinity;
    return this.a.snapshot(now);
  }
  restore(saved,{transports=false,demo=null}={}){
    if(demo&&this.t.session!==demo.ownerSession)return;
  ...
  cancelPractice({keepResult=false,preserveSticks=false}={}){
    ...
    if(s?.phase!=='practicing'&&!this.practiceCapture&&!this.ownedPlayback.size)return;
    if(this.practiceCapture?.session===s&&this.practiceCapture.revision===this.a.get(s.step.looperRole)?.looperData.takeRevision)this.finalizeCapture(s);
    const captured=this.a.get(s?.step?.looperRole);
    if(this.practiceCapture?.session===s&&captured?.timeline!==this.practiceCapture.timeline&&!captured?.transport.recording)
      this.a.takeEvidence[s.step.looperRole]=s.evidence.map(event=>({...event}));
    this.practiceCapture=null;
    for(const role of this.ownedPlayback)this.a.get(role)?.stop();
    this.ownedPlayback.clear();
  ...
  cancelOutgoing({keepResult=false,preserveSticks=false}={}){
    this.generation++;this.stopDemo(false);
    // Manual captures need the same ordinary Stop before navigation releases
    // input or a mode transition snapshots/disposes the current scene.
    for(const h of this.t.r.instrumentRegistry.getByKind('looper'))
      if(h.transport.recording||h.transport.recordArmed)this.t.r.pressLooperButton(h,'stop',null,performance.now());
    this.cancelPractice({keepResult,preserveSticks});
  ...
  async practice(){
    ...
    try{
      ...
      if(step.timed){
        ...
        const anchor=this.a.nextBoundary(now,4);
  ...
  async demonstrate(){
    ...
    this.t.demo={session,ownerSession:s,expectedTakes:{},stepId:step.id,savedTakes,savedHonks,checkpoint:{phase:s.phase,result:s.result,feedback:s.feedback},
    ...
    for(const role of Object.keys(savedTakes))this.rememberDemoCommand(role);
  ...
  stopDemo(completed=false,message=''){
    const demo=this.t.demo;if(!demo)return;
    const ownsState=this.t.session===demo.ownerSession&&this.demoOwnsTakes(demo);
    demo.conductor.stop();this.a.releaseVirtuals();this.t.demo=null;this.t.cues.reset();
    if(ownsState)for(const [id,defaults] of demo.savedHonks){const h=this.t.r.instrumentRegistry.get(id);if(h){h.setLivePerformance(defaults);h.setVowel(defaults.vowel);}}
    const saved=demo.savedMetro,metro=this.a.get('metronome'),now=performance.now();
    if(ownsState&&saved&&metro?.id===saved.id){if(metro.bpm!==saved.bpm)metro.setBpm(saved.bpm,now);metro.setVolume(saved.volume);if(saved.playing&&!metro.playing)metro.play(now);if(!saved.playing&&metro.playing)metro.pause(now);}
    this.restore(demo.savedTakes,{transports:true,demo});this.a.setVirtualsActive(false);
    const s=this.t.session;if(s===demo.ownerSession&&s?.mode==='practice'){
