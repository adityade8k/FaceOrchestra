...
import { COMPOSITION as C, PITCHES, TUTORIAL_PRESETS } from './composition.js';
...
export class TutorialAdapter {
  constructor(runtime, emit) {
    ...
    this.finalTimeline = null; this.lastTimeline = null; this.wasRecording = false;
  ...
  bindPreview(preview, entry, controller) {
    ...
    const role = preset?.role || (['metronome','looper'].includes(entry.id) ? entry.id : null);
    if (!role) return;
  ...
  addLabel(h,role) {
    ...
    const text = PITCHES[pitch] ? `${PITCHES[pitch].syllable} = ${pitch}` : group ? `${group.label}: ${group.notes}` : role;
    ...
    label.userData.isNoteLabel=true;label.scale.set(group?0.52:0.28,0.056,1);
    label.position.copy(h.root.position);label.position.y += 0.17;
  ...
  positionFor(role) {
    ...
    if (role==='looper') return p.add(new THREE.Vector3(0,-0.36,0.40));
  ...
  command(action, now, origin = this.origin) {
    const looper=this.get('looper'), metro=this.get('metronome');
    if (action==='clock-wire' && looper && metro) this.r.metronomeConnectionManager.connect({
      metronomeId:metro.id,portId:'port-0',targetKind:'looper',targetId:looper.id,targetPortId:looper.tracks[5].trackId });
    else if (action==='tempo' && metro && looper) { metro.setBpm(C.bpm,now);metro.pressButton('play',now);this.r.updateMetronomeLabel(metro);this.r.setLooperControlValue(looper,'gap',-1); }
    else if (action.startsWith('wire-') && looper) {
      const role=action.slice(5);const group=C.backing.find(g=>g.role===role);
      const h=this.get(role);if (h) this.r.connectLooperTrackToHonk(looper,group?.trackIndex ?? 4,h.id);
      ...
      if (looper) this.r.setLooperControlValue(looper,'volume',-0.55);
    } else if (action==='record' && looper) this.r.pressLooperButton(looper,'record');
    else if (action==='stop-record' && looper) this.r.pressLooperButton(looper,'stop');
    else if (action==='play' && looper) this.r.pressLooperButton(looper,'play');
    this.snapshotAt=-Infinity;
  ...
  observeStrike(event, {stick,target}, recordedCount) {
    ...
    else if(target.kind==='honk') lane=this.get('looper')?.tracks.find(t=>t.connectedHonkId===target.id)?.trackId;
    else if(target.kind==='metronome') lane=this.r.metronomeConnectionManager.getConnectionsForMetronome(target.id).find(c=>c.targetId===this.get('looper')?.id)?.targetPortId;
  ...
  observe(now) {
    ...
    for(const [id,label] of this.labels) {
      ...
      else {label.position.copy(h.root.position);label.position.y+=0.17;}
    }
  }
  midi(h) {
  ...
  snapshot(now) {
    ...
    if(!this.snapshotCache || now-this.snapshotAt>=100) {
      ...
      const wires={};
      for(const g of [...C.backing,{role:'percussion',trackIndex:4}]) wires[g.role]=Boolean(looper && looper.tracks[g.trackIndex]?.connectedHonkId===this.get(g.role)?.id);
      const clockWired=Boolean(metro && looper && this.r.metronomeConnectionManager.getConnectionsForMetronome(metro.id).some(c=>c.targetId===looper.id&&c.targetKind==='looper'&&c.targetPortId===looper.tracks[5].trackId));
      ...
      this.snapshotCache={roles,wires,clockWired,timbreReady};this.snapshotAt=now;
    }
    ...
    const recording=Boolean(looper?.transport.recording);
    if(looper?.timeline && !recording && (this.wasRecording || this.lastTimeline!==looper.timeline)) {
      this.finalTimeline=looper.timeline.hasRecording() ? looper.timeline.toJSON() : null;
      this.lastTimeline=looper.timeline;
    }
    this.wasRecording=recording;
    if(looper?.transport.playing && C.backing.some(g=>this.members(g.role).some(h=>h.hasAutomation() && h.getResolvedPerformanceState().squeeze>0.1))) this.playbackVoicesObserved=true;
    if(!looper?.transport.playing) this.playbackVoicesObserved=false;
    return {...this.snapshotCache,bpm:metro?.bpm,clockPlaying:Boolean(metro?.playing),tempoStableMs:this.tempoSince===null?0:now-this.tempoSince,
      gapBeats:looper?.looperData.gapBeats,recording,recordArmed:Boolean(looper?.transport.recordArmed),playing:Boolean(looper?.transport.playing),
      timeline:this.finalTimeline,liveGestures:this.gestures.size,anyStickContact:sticks.some(s=>s.contactTargetIds.size>0),
      ...
      audioRunning:this.r.audioSystem.audioContextService.context?.state==='running',playbackVoicesObserved:this.playbackVoicesObserved};
  }
  nextBoundary(now, beats=1, countIn=4) {
    const metro=this.get('metronome'), looper=this.get('looper');
  ...
  stopSound() { this.get('looper')?.stop();this.get('metronome')?.pause();this.r.audioSystem.releaseAll(); }
  clear() {
    ...
    this.tempoSince=null;this.snapshotCache=null;this.finalTimeline=null;this.lastTimeline=null;this.wasRecording=false;
