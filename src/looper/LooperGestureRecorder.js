...
export class LooperGestureRecorder {
  ...
  start(timeline, tracks, now, captureAction) {
    timeline.startRecording(now);
    for (const track of tracks) {
      track.resetRecordingState();
      const captured = captureAction(track.connectedHonkState);
  ...
  updateTrack(timeline, track, now, captureAction) {
    ...
    if (!track.recorderState.hasBaseline) {
      track.recorderState.baseline = this.cloneActionState(action);
  ...
  addContinuousFieldEvent(timeline, track, field, elapsedMs, value) {
    if (field === "squeeze") {
      const previous = track.recorderState.recordedFields.has("squeeze")
        ...
        : 0;
      const type =
        previous <= SQUEEZE_GATE_EPSILON && value > SQUEEZE_GATE_EPSILON
  ...
  cloneActionState(source) {
    return cloneActionState(source);
  }
}
