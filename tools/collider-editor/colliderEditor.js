...
import { STICK_SETTINGS } from "../../src/config/stick.js";
import { applyStandardInstrumentMaterials, loadMaterialTextureSet } from "../../src/scene/materialUtils.js";
import {
  getLooperButtonName,
  getLooperControlName,
  getLooperNodeName,
} from "../../src/instruments/looper/looperNames.js";
...
function resolveRepositoryPath(path) {
  const repositoryRelativePath = String(path || "").replace(/^\.\//, "");
  return new URL(`../../${repositoryRelativePath}`, import.meta.url).href;
}

function resolveRepositoryTexturePaths(texturePaths) {
  return Object.fromEntries(
    Object.entries(texturePaths).map(([name, path]) => [name, resolveRepositoryPath(path)]),
  );
}
...
const objectDefinitions = {
  honk: {
    label: "Honk",
    modelPath: resolveRepositoryPath(ASSET_PATHS.models.honk),
    texturePaths: resolveRepositoryTexturePaths(ASSET_PATHS.textures.honk),
  },
  looper: {
    label: "Looper",
    modelPath: resolveRepositoryPath(ASSET_PATHS.models.looper),
    texturePaths: resolveRepositoryTexturePaths(ASSET_PATHS.textures.looper),
  },
  stick: {
    label: "Stick",
    modelPath: resolveRepositoryPath(ASSET_PATHS.models.stick),
    texturePaths: resolveRepositoryTexturePaths(ASSET_PATHS.textures.stick),
  },
};
